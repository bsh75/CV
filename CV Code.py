...
def init_file():
    """Load the video file"""
    cap = cv2.VideoCapture('./Snip.avi')  # Chnge to 0 instead of filename to get from camera'./Snip.avi'   './Data/MovHotspot.mp4'
...
def main():
    ...
    threshold1 = 70 # Exact value will be determined during later testing
    ...
    while cap.isOpened():
        ...
        """Instead could check whether the size of the summed target point brightest values"""
        maxSum = 0
        targetLoc = []
        j = 0
        ...
        """for i in range(0, len(maxContours)):
            targetLoc.append(targetPoint(ze_frame, blurKsize))
            # Inner loop to check if target lies in any of the max N contours
            for j in range(0, len(maxContours)):
                if cv2.pointPolygonTest(maxContours[j], targetLoc[i], True):
                    cv2.circle(ze_frame, targetLoc[i], radius, (0, 0, 255), cv2.FILLED)
        #targetLoc.append(targetPoint(ze_frame, blurKsize))"""
        ...
        distanceBetween(targetLoc, width, height, camAngleX, camAngleY)
        ...
        for k in range(0, len(targetLoc)):
            ...
            cv2.putText(frameOut, str(dGuess)+'cm '+str(round(theta[k]*180/np.pi, 1))+'deg', (targetLoc[k][0]+30, targetLoc[k][1]+0),cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 3)
