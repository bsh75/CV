...
def init_file():
    ...
    # deviceIndex = file
    deviceIndex = Y16TestData[1][1] # Device Index determines what the code is run on. 0 = Webcam. 1 = USB port on computer
...
def main():
    ...
    rawThreshAvg = 0 # For determining if the blurred area is significant enough to target
    distThresh = 0.5 # Distance threshold for target drop trigger

    # Read the raw Y16 data from the camera
    cap, device = init_file()
    ...
    if save:
        filename = './drawOnCnt/'+device.split('/')[1]+device.split('/')[2]
